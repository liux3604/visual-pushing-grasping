...
def checkEmptyTable(args):
    ''' Reset simulation or pause real-world training if table is empty by inspecting the table image according to an empty_threshold. If yes, reposition the objects.
    ...
    if args.is_sim and args.is_testing:
        empty_threshold = 10
    if np.sum(stuff_count) < empty_threshold or (args.is_sim and shared.no_change_count[0] + shared.no_change_count[1] > 10):
        shared.no_change_count = [0, 0]
        if args.is_sim:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tcp_host_ip', dest='tcp_host_ip', action='store',
                        default='100.127.7.223', help='IP address to robot arm as TCP client (UR5)')
    parser.add_argument('--tcp_port', dest='tcp_port', type=int, action='store',
                        default=30002, help='port to robot arm as TCP client (UR5)')
    parser.add_argument('--rtc_host_ip', dest='rtc_host_ip', action='store',
                        default='100.127.7.223', help='IP address to robot arm as real-time client (UR5)')
    ...
    parser.add_argument('--random_seed', dest='random_seed', type=int, action='store',
                        default=1234, help='random seed for simulation and neural net initialization')
