...
class Robot(object):
    def __init__(self, is_sim, obj_mesh_dir, obj_model_dir, load_models, num_obj, workspace_limits,
        ...
        if self.is_sim:
            ...
            # There is already has a default remote API server
            # running at 19997. Its configuration can be modified at remoteApiConnections.txt
    ...
    def move_to(self, tool_position, tool_orientation):
        ...
        if self.is_sim:
            # Each move_to action takes 2.5 seconds. 1/0 02*50ms = 2.5s
            sim_ret, UR5_target_position = vrep.simxGetObjectPosition(
                self.sim_client, self.UR5_target_handle, -1, vrep.simx_opmode_blocking)
