        ...
        while True:
            ...
            if not self.args.grasp_only:
                ...
                if explore_actions:
                    ...
                    shared.primitive_action = 'push' if np.random.randint(0, 2) == 0 else 'grasp'
                else:
            ...
            if self.args.heuristic_bootstrap and shared.primitive_action == 'push' and shared.no_change_count[0] >= 2:
                ...
                shared.best_pix_ind = self.trainer.push_heuristic(shared.valid_depth_heightmap)
                ...
                shared.best_pix_ind = self.trainer.grasp_heuristic(shared.valid_depth_heightmap)
                ...
                if shared.primitive_action == 'push':
                    shared.best_pix_ind = np.unravel_index(np.argmax(shared.push_predictions), shared.push_predictions.shape)
            ...
            print('Action: %s at (%d, %d, %d)' % (shared.primitive_action, shared.best_pix_ind[0], shared.best_pix_ind[1], shared.best_pix_ind[2]))
            best_rotation_angle = np.deg2rad(shared.best_pix_ind[0]*(360.0/self.trainer.model.num_rotations))
            best_pix_x = shared.best_pix_ind[2]
            best_pix_y = shared.best_pix_ind[1]
            primitive_position = [best_pix_x * self.args.heightmap_resolution + self.args.workspace_limits[0][0], 
                                  best_pix_y * self.args.heightmap_resolution + self.args.workspace_limits[1][0], 
                                  shared.valid_depth_heightmap[best_pix_y][best_pix_x] + self.args.workspace_limits[2][0]]

            # If pushing, adjust start position, and make sure z value is safe and not too low
            # or shared.primitive_action == 'place':
            if shared.primitive_action == 'push':
                finger_width = 0.02
                safe_kernel_width = int(np.round((finger_width/2)/self.args.heightmap_resolution))
                ...
                if local_region.size == 0:
                    ...
                    safe_z_position = np.max(local_region) + self.args.workspace_limits[2][0]
                primitive_position[2] = safe_z_position

            # Save executed primitive
            if shared.primitive_action == 'push':
                self.trainer.executed_action_log.append([0, shared.best_pix_ind[0], shared.best_pix_ind[1], shared.best_pix_ind[2]])  # 0 - push
            elif shared.primitive_action == 'grasp':
                self.trainer.executed_action_log.append([1, shared.best_pix_ind[0], shared.best_pix_ind[1], shared.best_pix_ind[2]])  # 1 - grasp
